try {
	...
	if (slide == 2) {
		...
		if ((text_selected != "chapter") || (custom != 2)) {
		...
		if (popup == "") {
			...
			draw_sprite(spr_creation_check, yar, 519, 239);
			yar = 0;
			if (scr_hit(519, 239, 519 + 32, 239 + 32) && (cooldown <= 0) && (mouse_left >= 1) && (custom == 2)) {
				cooldown = 8000;
				if ((points + 20 <= maxpoints) && (fleet_type == 3)) {
					points += 20;
					fleet_type = 1;
				}
				if (fleet_type == 2) {
			...
			draw_sprite(spr_creation_check, yar, 771, 239);
			yar = 0;
			if (scr_hit(771, 239, 771 + 32, 239 + 32) && (cooldown <= 0) && (mouse_left >= 1) && (custom == 2)) {
				cooldown = 8000;
				if ((points + 20 <= maxpoints) && (fleet_type == 3)) {
					points += 20;
					fleet_type = 2;
			...
			draw_sprite(spr_creation_check, yar, 958, 239);
			yar = 0;
			if (scr_hit(958, 239, 958 + 32, 239 + 32) && (cooldown <= 0) && (mouse_left >= 1) && (custom == 2)) {
				if (fleet_type != 3) {
					points -= 20;
				}
				fleet_type = 3;
			...
			for (var i = 0; i < 4; i++) {
				if (custom == 2) {
					draw_sprite_stretched(spr_arrow, 0, 625, 325 + (i * 55), 32, 32);
				}
				if (scr_hit(625, 325 + (i * 55), 657, 357 + (i * 55))) {
					obj_cursor.image_index = 1;
					if ((cooldown <= 0) && (custom == 2) && (arrow_buttons_controls[i] > 1) && (mouse_left >= 1)) {
				...
				if (custom == 2) {
					draw_sprite_stretched(spr_arrow, 1, 1135, 325 + (i * 55), 32, 32);
				}
				if (scr_hit(1135, 325 + (i * 55), 1167, 357 + (i * 55))) {
					obj_cursor.image_index = 1;
					if ((cooldown <= 0) && (custom == 2) && (arrow_buttons_controls[i] < 10) && (points + 10 <= maxpoints) && (mouse_left >= 1)) {
						arrow_buttons_controls[i] += 1;
						points += 10;
						cooldown = 8000;
					}
			...
			if (scr_hit(532, 325, 1166, 357)) {
				tooltip = "Strength";
				tooltip2 = "How many companies your chapter begins with.  For every score below five a company will be removed; conversely, each score higher grants 50 additional astartes.";
			}
			if (scr_hit(486, 380, 1166, 412)) {
				tooltip = "Cooperation";
				tooltip2 = "How diplomatic your chapter is.  A low score will lower starting dispositions of Imperial factions and make disposition increases less likely to occur.";
			}
			if (scr_hit(442, 435, 1166, 467)) {
				tooltip = "Purity";
				tooltip2 = "A measure of how pure and mutation-free your chapter's gene-seed is.  A perfect score means no mutations must be chosen.  The lower the score, the more mutations.";
			}
			if (scr_hit(423, 490, 1166, 522)) {
				tooltip = "Stability";
				tooltip2 = "A measure of how easily new mutations and corruption can occur with your chapter-gene seed.  A perfect score makes the gene-seed almost perfectly stable.";
			}
		}
		...
		if (popup != "icons") {
			...
			var adv_txt = {x1: 436, y1: 570, w: 204, h: 20};
			...
			for (i = 1; i <= max_advantage_count; i++) {
				...
				if (scr_hit(adv_txt.x1, adv_txt.y1 + (i * adv_txt.h), adv_txt.x2, adv_txt.y2 + (i * adv_txt.h))) {
					if ((points >= maxpoints) && (adv_num[i] == 0) && (popup == "") && (custom > 1)) {
			...
			var dis_txt = {x1: 810, y1: 570, w: 204, h: 20};
			...
			for (var slot = 1; slot <= max_disadvantage_count; slot++) {
				...
				if (scr_hit(dis_txt.x1, dis_txt.y1 + (slot * dis_txt.h), dis_txt.x2, dis_txt.y2 + (slot * dis_txt.h))) {
					...
					if (advantage_click) {
						...
						if ((mouse_x <= 830) && removable && (points + cur_dis.points <= maxpoints)) {
							var cur_dis = obj_creation.all_disadvantages[dis_num[slot]];
							cur_dis.remove(slot);
			...
			repeat (24) {
				...
				if (ic <= total_icons) {
					...
					if ((ic >= normal_and_builtin) && (obj_cuicons.spr_custom[ic - normal_and_builtin] > 0) && (obj_cuicons.spr_custom_icon[ic - normal_and_builtin] != -1)) {
			...
			y3 = top;
			y4 = y3 + (24 * see_size) - see_size;

			if (scrollbar_engaged == 0) {
				draw_rectangle(x3, y3 + bs, x4, y4 + bs, 0);
			}
			...
			if (scr_hit(x3, y3 + bs, x4, y4 + bs) && (cooldown <= 0) && (scrollbar_engaged <= 0) && (mouse_left == 1)) {
				// Click within the scrollbar grip area
		...
		if (popup == "advantages") {
			...
			for (var slot = 0; slot < array_length(obj_creation.all_advantages); slot++) {
				...
				var column = {x1: 436, y1: 250, w: 100, h: 20};
				...
				if (advantage_local_var.name != "") {
					...
					if (point_and_click(coords) && (cooldown <= 0) && (adv_name == "Cancel")) {
						cooldown = 8000;
						popup = "";
					}
					// Tooltips
					...
					//Click on advantage
					if (point_and_click(coords) && (cooldown <= 0) && (array_contains(adv, adv_name) == false)) {
						if (disable == 0) {
							cooldown = 8000;
			...
			for (var slot = 0; slot < array_length(obj_creation.all_disadvantages); slot++) {
				...
				if (disadvantage_local_var.name != "") {
					...
					if (point_and_click(coords) && (cooldown <= 0) && (dis_name == "Cancel")) {
						cooldown = 8000;
						popup = "";
					}
					//Tooltip
					...
					if (point_and_click(coords) && (cooldown <= 0) && (array_contains(dis, dis_name) == false)) {
						if (disable == false) {
							cooldown = 8000;
							popup = "";
		...
		if ((popup != "") && ((mouse_left >= 1) || (mouse_right == 1)) && (cooldown <= 0)) {
			if (((mouse_x < 445) || (mouse_x > 1125) || (mouse_y < 200) || (mouse_y > 552)) && (popup != "icons")) {
				cooldown = 8000;
				popup = "";
			}
			if (((mouse_x < 445) || (mouse_x > 1125) || (mouse_y < 200) || (mouse_y > 719)) && (popup == "icons")) {
				cooldown = 8000;
				popup = "";
			}
		}
	...
	if (slide == 3) {
		...
		if (fleet_type == eFLEET_TYPES.HOMEWORLD) {
			...
			if ((text_selected != "home_name") || (custom < 2)) {
				draw_text_transformed(644, 398, string_hash_to_newline(string(homeworld_name)), 0.5, 0.5, 0);
			}
			if (custom > 1) {
				if ((text_selected == "home_name") && (text_bar > 30)) {
					draw_text_transformed(644, 398, string_hash_to_newline(string(homeworld_name)), 0.5, 0.5, 0);
				}
				if ((text_selected == "home_name") && (text_bar <= 30)) {
					draw_text_transformed(644, 398, string_hash_to_newline(string(homeworld_name) + "|"), 0.5, 0.5, 0);
			...
			var planet_change_allow = (mouse_left >= 1) && (cooldown <= 0) && (custom > 1);
		...
		if (fleet_type != eFLEET_TYPES.HOMEWORLD) {
			...
			// draw_text_transformed(644,398,string(homeworld_name),0.5,0.5,0);
			if ((text_selected != "flagship_name") || (custom == 0)) {
				draw_text_transformed(644, 398, string_hash_to_newline(string(flagship_name)), 0.5, 0.5, 0);
			}
			if (custom > 1) {
		...
		if (fleet_type != eFLEET_TYPES.PENITENCE) {
			...
			yar = 0;
			if (scr_hit(858, 221, 858 + 32, 221 + 32) && (cooldown <= 0) && (mouse_left >= 1) && (custom > 1) && (fleet_type == 1)) {
				cooldown = 8000;
				var onceh;
				onceh = 0;
				if ((recruiting_exists == 1) && (onceh == 0)) {
					recruiting_exists = 0;
					onceh = 1;
				}
				if ((recruiting_exists == 0) && (onceh == 0)) {
					recruiting_exists = 1;
					onceh = 1;
				}
			}
			draw_set_alpha(1);
			draw_text_transformed(644 + 333, 218, string_hash_to_newline("Recruiting World"), 0.6, 0.6, 0);

			if (recruiting_exists == 1) {
				if (recruiting == "Lava") {
					eh2 = 2;
				}
				if (recruiting == "Desert") {
					eh2 = 3;
				}
				if (recruiting == "Forge") {
					eh2 = 4;
				...
				if (recruiting == "Feudal") {
					eh2 = 8;
				}
				if (recruiting == "Temperate") {
					eh2 = 9;
				}
				if (recruiting == "Ice") {
					eh2 = 10;
				...
				if (scr_hit(865, 285, 865 + 32, 285 + 32) && (mouse_left >= 1) && (cooldown <= 0) && (custom > 1)) {
					var onceh;
					onceh = 0;
					cooldown = 8000;
					if ((recruiting == "Dead") && (onceh == 0)) {
						recruiting = "Ice";
						onceh = 1;
					}
					if ((recruiting == "Ice") && (onceh == 0)) {
						recruiting = "Temperate";
						onceh = 1;
					}
					if ((recruiting == "Temperate") && (onceh == 0)) {
						recruiting = "Feudal";
						onceh = 1;
					}
					if ((recruiting == "Feudal") && (onceh == 0)) {
						recruiting = "Shrine";
						onceh = 1;
					}
					if ((recruiting == "Shrine") && (onceh == 0)) {
						recruiting = "Agri";
						onceh = 1;
					}
					if ((recruiting == "Agri") && (onceh == 0)) {
						recruiting = "Death";
						onceh = 1;
					}
					if ((recruiting == "Death") && (onceh == 0)) {
						recruiting = "Hive";
						onceh = 1;
					}
					if ((recruiting == "Hive") && (onceh == 0)) {
						recruiting = "Forge";
						onceh = 1;
					}
					if ((recruiting == "Forge") && (onceh == 0)) {
						recruiting = "Desert";
						onceh = 1;
					}
					if ((recruiting == "Desert") && (onceh == 0)) {
						recruiting = "Lava";
						onceh = 1;
					}
					if ((recruiting == "Lava") && (onceh == 0)) {
						recruiting = "Dead";
						onceh = 1;
					}
				}
				...
				if (scr_hit(1055, 285, 1055 + 32, 285 + 32) && (mouse_left >= 1) && (cooldown <= 0) && (custom > 1)) {
					var onceh;
					onceh = 0;
					cooldown = 8000;
					if ((recruiting == "Dead") && (onceh == 0)) {
				...
				if ((fleet_type == 1) && (homeworld_name == recruiting_name)) {
					name_bad = 1;
				}
				...
				if ((text_selected != "recruiting_name") || (custom < 2)) {
					draw_text_transformed(644 + 333, 398, string_hash_to_newline(string(recruiting_name)), 0.5, 0.5, 0);
				}
				if (custom > 1) {
		...
		if (fleet_type == eFLEET_TYPES.HOMEWORLD) {
			...
			if (scr_hit(445, 512, 445 + 32, 512 + 32) && (cooldown <= 0) && (mouse_left >= 1) && (custom > 1) && (homeworld_rule != 1)) {
				cooldown = 8000;
				homeworld_rule = 1;
			}
			...
			if (scr_hit(445, 544, 445 + 32, 544 + 32) && (cooldown <= 0) && (mouse_left >= 1) && (custom > 1) && (homeworld_rule != 2)) {
				cooldown = 8000;
				homeworld_rule = 2;
			}
			...
			if (point_and_click([445, 576, 445 + 32, 576 + 32]) && (custom > 1) && (homeworld_rule != 3)) {
				cooldown = 8000;
				homeworld_rule = 3;
			}
		...
		if (custom > 1) {
			draw_sprite_stretched(spr_creation_arrow, 0, 0, 80, 32, 32);
			if (point_and_click([0, 80, 0 + 32, 80 + 32]) && (cooldown <= 0)) {
				var onceh = 0;
				cooldown = 8000;
				aspirant_trial++;
			...
			if (point_and_click([38, 80, 38 + 32, 80 + 32]) && (mouse_left >= 1) && (cooldown <= 0)) {
				var onceh = 0;
				cooldown = 8000;
				aspirant_trial--;
				if (aspirant_trial < 0) {
		...
		if (race[100, 17] != 0) {
			...
			var fug, fug2;
			fug = string_delete(discipline, 2, string_length(discipline));
			fug2 = string_delete(discipline, 1, 1);
			...
			if (scr_hit(437, 688, 437 + 32, 688 + 32) && (mouse_left >= 1) && (cooldown <= 0) && (custom > 1)) {
				var onceh;
				onceh = 0;
				cooldown = 8000;
				if ((discipline == "default") && (onceh == 0)) {
					discipline = "rune Magick";
					onceh = 1;
				}
				if ((discipline == "rune Magick") && (onceh == 0)) {
					discipline = "telekinesis";
					onceh = 1;
				}
				if ((discipline == "telekinesis") && (onceh == 0)) {
					discipline = "pyromancy";
					onceh = 1;
				}
				if ((discipline == "pyromancy") && (onceh == 0)) {
					discipline = "biomancy";
					onceh = 1;
				}
				if ((discipline == "biomancy") && (onceh == 0)) {
					discipline = "default";
					onceh = 1;
				}
			}
			if (scr_hit(475, 688, 475 + 32, 688 + 32) && (mouse_left >= 1) && (cooldown <= 0) && (custom > 1)) {
				var onceh;
				onceh = 0;
				cooldown = 8000;
				if ((discipline == "default") && (onceh == 0)) {
